...
export default function ProductPage() {
  let { Customer_detail, token, isAuth } = useSelector(
  ...
  const { loading: isBookingLoading } = useSelector(
    (state) => state?.BookingManager
  );
  const { loading: isOneCarLoading } = useSelector(
  ...
  const isSlotAvailable = (slot) => {
    for (const inst of OtherTestDrives) {
      if (
        inst.test_drive_date == TDriveData.test_drive_date &&
        inst.test_drive_slot == slot
      )
        return true;
    }
    return false;
  ...
  const CanUserBookNow = () => {
    ...
    for (const TD of OtherTestDrives) {
      if (
        TD?.customer_id?._id == Customer_detail?._id &&
        TD.status == "approved"
      ) {
        flag = true;
      }
  ...
  const UpdateBookingAndCarIsSold = (obj) => {
    ...
    dispatch(
      UpdatebookingByID(
        ...
        getData,
        onPayClose,
        "You have successfully bought the car",
        customertoken
      )
    );
  };
    ...
    <Card shadow="none" mx={{ base: '5', md: '5' }} my={{ base: '5', md: '5' }} >
      ...
      {!isOneCarLoading ? (
        ...
        >
            ...
            IsUserTheOneWhoBookedCar() && (
              <>
                <Alert status="info">
                  <AlertIcon />
                  <AlertTitle>
                    This Car was booked by you on{" "}
                    {formatCreatedAtDate(PrevBookData?.created_at)}.{" "}
                  </AlertTitle>
                </Alert>
                <Button
                  colorScheme="blue"
          ...
          {data?.booking_status == "booked" &&
            IsUserTheOneWhoBookedCar() == false && (
              <>
                <Alert status="info">
                  <AlertIcon />
...
<Flex>
      ...
      <GallerySlider
        ...
        onClose={closeGallerySlider}
        data={data?.gallery_images}
        ind={galleryImageIndex}
      />
            ...
            <ModalBody>
              ...
              >
                ...
                <FormControl
                  isRequired
                  isInvalid={
                    TDriveData?.phone_number?.length !== 10 &&
                    TDriveData?.phone_number?.length > 0
                  }
                >
              ...
              >
                <FormControl isRequired>
                  ...
                  <Input
                    defaultValue={TDriveData?.address || ""}
                    placeholder="Address"
                    name="address"
                    onChange={handleInputChange}
                  />
              ...
              >
                ...
                <FormControl isRequired>
                  ...
                  <Input
                    defaultValue={TDriveData?.state?.state_name || ""}
                    placeholder="State"
                    name="state"
                    onChange={handleInputChange}
                  />
                ...
                <FormControl isRequired>
                  ...
                  <Input
                    defaultValue={TDriveData?.pincode || ""}
                    type="number"
                    placeholder="PIN"
                    name="pincode"
                    onChange={handleInputChange}
                ...
                <FormControl isRequired>
                  ...
                  <InputUpload
                    ...
                    HandleUploadSomeImages={(file) =>
                      setTDriveData({ ...TDriveData, driving_license: file })
                    }
                    refresh={refresh}
                  />
                </FormControl>
      ...
      >
        ...
        <ModalContent>
          <ModalHeader fontWeight={"semibold"}>
            Pay Rest 10% of Car's Price to confirm Booking
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody px="10">
      ...
      >
        ...
        <ModalContent>
          <ModalHeader fontWeight={"semibold"}>
            Pay Rest 90% to confirm buying the Car
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody px="10">
            <Text mb="3"> Total Price of Car : ₹ {data?.price}</Text>
            <Text mb="3">
              {" "}
              Booking Charges : 10% of Total Price of Car (Paid)
            </Text>
            <Text mb="3">
              Booking Charges Amount : ₹ {data?.price * 0.1 || 0} (Paid)
            </Text>
            <Text mb="3">
              {" "}
              Amount Paid : ₹ {data?.price * 0.1 + 500}/- (Test Drive Charge
              Incl.)
            </Text>
            <Text>
              Amount Left to be Paid : ₹{" "}
              {data?.price - (data?.price * 0.1 + 500)}
            </Text>
            <br />
            <FormControl isRequired>
              <FormLabel>Payable Amount :</FormLabel>
