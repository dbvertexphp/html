...
export default function Header() {
  ...
  const { Customer_detail, token } = useSelector(
  ...
  return (
    <>
      ...
      >
        <NavLink to="/">
          <Image src={logo} width="180px" marginLeft={{ md: "30px" }} />
        </NavLink>
      ...
      {isOpen && (
        <Box p={3} pr="10" display={{ md: "none" }} bg="gray.50">
          <Stack
            as={"nav"}
            spacing={4}
            onClick={menuHandler}
            fontWeight={"bold"}
          >
            <NavLink to="/collection"> Buy Cars </NavLink>
            <NavLink to="/vendor-register"> Sell Cars </NavLink>
            ...
            {!token && !Customer_detail ? (
              ...
              <NavLink to={"/customer"}>
                <Avatar size={"xs"} src={Customer_detail?.profile_pic} />
                Hello{" "}
                {Customer_detail?.first_name + " " + Customer_detail?.last_name}
              </NavLink>
            )}
          </Stack>
        </Box>
