...
import { DeleteCarByID, getCarByID, getCarsByVendorID } from '../../Redux/App/Actions/Vendors/Car.action';
...
const VendorCar = () => {
  ...
  const getData = () => {
    let data = { sortby };
    dispatch(getCarsByVendorID(Vendor_detail?._id, page, data, setVendorCars, vendortoken));
  };
    ...
    <Box background={'white'} p="5" borderRadius="5px">
      ...
      <HStack py={'10px'} justifyContent={'space-between'} alignContent={'center'}>
        ...
        <Link to="/vendor/cars/add-car">
          <Button bg="#30829c" colorScheme="blue" variant={'solid'} rightIcon={<FiPlusCircle />}>
            Add car
          </Button>
        </Link>
      </HStack>
