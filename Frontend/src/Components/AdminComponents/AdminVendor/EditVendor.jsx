...
const EditVendor = () => {
  ...
  const SaveVendorDetails = async () => {
    ...
    dispatch(UpdateVendorByID(params.id, formData, toast, navigate, '/admin/vendor', getData, title, admintoken));
  };

  const getData = id => {
    dispatch(getVendorByID(id, setFormData, toast, navigate, admintoken));
  };
  ...
  useEffect(() => {
    let id = params.id;
    if (!id) return navigate('/admin/vendor');
    ...
    <Box padding={'20px'}>
      <Container as="form" maxW="container" borderRadius="5px" mb="10" padding={'20px'} backgroundColor={'white'}>
        ...
        <Flex gap={1} justifyContent={'end'} my={'15px'}>
          ...
          <Button colorScheme="blue" m="10px" rightIcon={<FiCheckCircle />} onClick={SaveVendorDetails}>
            Save Vendor Details
          </Button>
        </Flex>
      </Container>
