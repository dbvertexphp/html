...
const AdminVendor = () => {
  ...
  const ChangeOrderStatusFunction = (id, data) => {
    ...
    dispatch(UpdateVendorByID(id, data, toast, navigate, '/admin/vendor', getData, title, admintoken));
  };
  const closeModal = () => {
    ...
    <>
      <Container maxW="container" borderRadius="5px" minH={'610px'} padding={'20px'} backgroundColor={'white'}>
        ...
        <HStack py={'10px'} justifyContent={'space-between'} alignContent={'center'}>
          <InputGroup w={'full'}>
            <Input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={'Search by Vendor Details'} />
          ...
          <Button bg="#30829c" color="white" variant={'solid'} w={'22%'} onClick={refreshAll}>
            Refresh
          </Button>

          <Link to={'/admin/vendor/add-vendor'}>
            <Button bg="#30829c" color="white" variant={'solid'} rightIcon={<FiPlusCircle />}>
              Add Vendor
            </Button>
          </Link>
        </HStack>
                      ...
                      <Td sx={cellStyle}>
                        <Flex gap={1} opacity={'0.9'}>
                          <Button
                            ...
                            style={{ padding: '0px' }}
                            size={'xs'}
                            onClick={() => navigate(`/admin/vendor/edit/${item._id}`)}
                          >
                            <FiEdit3 />
                          </Button>
                          <Button
      ...
      {/**<!--*------- <Disable Poput> ----------->*/}
      <AlertDialog isCentered isOpen={isOpen1} leastDestructiveRef={cancelRef} onClose={onClose}>
        <AlertDialogOverlay>
          <AlertDialogContent>
            <AlertDialogHeader fontSize="lg" fontWeight="bold">
              Disable Vendor
            ...
            <AlertDialogBody>You are temporarily disabling this Vendor</AlertDialogBody>
