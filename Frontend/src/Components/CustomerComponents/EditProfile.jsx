import { Avatar, Button, Card, Flex, FormControl, FormLabel, Heading, Input, Stack, Text, useToast } from '@chakra-ui/react';
import { useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import InputUpload from '../Extra/InputUpload';
import { UpdateCustomerByID, getCustomerByID } from '../../Redux/App/Actions/Customer.action';
...
export default function EditProfile() {
  ...
  const submitHandler = () => {
    setIsEditing(false);
    dispatch(UpdateCustomerByID(customerDetails?._id, formData, toast, getData, 'User Details Updated Successfully!', customertoken));
  };
      ...
      <Card p="5" border="1px solid #ddd">
        <Flex gap="3" flexDirection={{ base: 'column', md: 'row' }} mb="3">
          ...
          <FormControl>
            ...
            <InputUpload isDisabled={!isEditing} HandleUploadSomeImages={HandleUploadSomeImages} accept={'image/*'} />
          </FormControl>
        </Flex>
        <Flex gap="3" flexDirection={{ base: 'column', md: 'row' }} mb="3">
          <FormControl>
            <FormLabel>Email :</FormLabel>
            <Input type="email" placeholder={isEditing ? 'Enter Email' : ' Email'} name="email" isDisabled value={customerDetails?.email || ''} />
          </FormControl>
          <FormControl>
        ...
        <Flex gap="3" mb="3" justifyContent={'end'} mt="5">
          {!isEditing && <Button onClick={editHandler}>EDIT DETAILS</Button>}
          {isEditing && (
