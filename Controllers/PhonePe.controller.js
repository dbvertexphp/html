...
const { BookingSuccessAdvanceCustomer, BookingSuccessAdvanceAdmin, BookingSuccessFullCustomer, BookingSuccessFullAdmin } = require('../Email/BookingTemplates/BookingTemplates');
...
exports.PhonePeCallbackHandler = async (req, res) => {
    ...
    let { code, transactionId } = req.body
    ...
    try {
        ...
        let { transaction_type, car_id, test_drive_id } = transaction
        ...
        if (code == "PAYMENT_SUCCESS") {
            ...
            if (transaction_type === "Test Drive Booking") {
                ...
                SendMail({ recipientEmail: newTD?.email, subject: "Test Drive Created", html: TestDriveBookingUser(newTD) })
                SendMail({ recipientEmail: "", subject: "New Test Drive Booked", html: TestDriveBookingAdmin({ TestDrive: newTD }) })
                ...
                car.booking_code = payload?.booking_code
                payload.payment_status = "partially_paid"

                await CarModel.findByIdAndUpdate(payload.car_id, car)

                let newBooking = new BookingModel(payload)
                let customer = await CustomerModel.findById(newBooking?.customer_id)

                customer?.email && SendMail({ recipientEmail: customer?.email, subject: "Booking Advance Payment Recieved", html: BookingSuccessAdvanceCustomer(car, customer) })
                SendMail({ recipientEmail: "", subject: "Booking Advance Payment Recieved", html: BookingSuccessAdvanceAdmin(car) })
                ...
                let booking = await BookingModel.findByIdAndUpdate(payload.booking_id, { remaining_amount: 0, payment_status: "paid" });
                let customer = await CustomerModel.findById(booking?.customer_id)

                customer?.email && SendMail({ recipientEmail: customer?.email, subject: "Booking Advance Payment Recieved", html: BookingSuccessFullCustomer(car, customer) })
                SendMail({ recipientEmail: "", subject: "Booking Advance Payment Recieved", html: BookingSuccessFullAdmin(car, customer) })
