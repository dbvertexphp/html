...
exports.getAllVendorsByEmployee = async (req, res) => {
  ...
  try {
    const vendors = await VendorModel.find({ reference: id });

    const carsPromises = vendors.map(async vendor => {
      const carCount = await CarModel.find({ vendorID: vendor._id }).count();
      return { ...vendor.toObject(), carCount };
    });
...
exports.getAllCarsByEmployeeID = async (req, res) => {
  ...
  try {
    ...
    for (const vendor of Vendors) {
      ...
      const cars = await CarModel.find(query).limit(limit).skip(skip).sort(sortObj);
...
exports.getAllBookingByEmployeeID = async (req, res) => {
  ...
  try {
    ...
    for (const vendor of Vendors) {
      ...
      const cars = await BookingModel.find(query).limit(limit).skip(skip).sort(sortObj);
