...
exports.getAllTransactions = async (req, res) => {
    ...
    try {
        ...
        const transactions = await TransactionModel.find(query).sort({ createdAt: -1 }).limit(limit).skip(skip)
            .populate(["car_id", "test_drive_id", "customer_id", "car_id.vendorID"]).populate(populateObj).exec();
