...
exports.UpdateBookingByID = async (req, res) => {
  ...
  try {
    ...
    if (payload?.car_status == 'sold') {
      let car = await CarModel.findOne({ _id: payload.car_id });
      let vendorID = car.vendorID;
      let vendor = await VendorModel.findOne({ _id: vendorID });
      let commissionType = vendor?.commission_type;
      let commissionAmount = vendor?.commission_amount;
      let caramount = car.price;
      let transactionAmount = null;

      if (commissionType === 'Percentage_Based' && commissionAmount) {
        let commissionPercent = commissionAmount / 100;
        let commissionValue = caramount * commissionPercent;
        transactionAmount = commissionValue;
      ...
      let newTransaction = {
        ...
        vendor_pay_amount: transactionAmount
      };
